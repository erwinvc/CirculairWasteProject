...
namespace Valve.VR.InteractionSystem {
    ...
    public class LinearDrive : MonoBehaviour {
        ...
        [Range(0.0f, 1.0f)] public float springToPosition = 0.5f;
        [Range(0.1f, 10.0f)] public float springSpeed = 2.0f;
        ...
        protected virtual void Update() {
            if (springBack && !grabbed) {
                mappingChangeRate = Mathf.Lerp(mappingChangeRate, 0.0f, momemtumDampenRate * Time.deltaTime);
                linearMapping.value = Mathf.Clamp01(Mathf.Lerp(linearMapping.value, springToPosition, Time.deltaTime * springSpeed));
                if (repositionGameObject) {
                    transform.position = Vector3.Lerp(startPosition.position, endPosition.position, linearMapping.value);
                }
            }
